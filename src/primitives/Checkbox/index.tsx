...
function Checkbox({
  ...
  const styles = React.useMemo(() => getCheckboxStyles({ isChecked: isCheckedProp, error, disabled, isIndeterminate }), [isCheckedProp, error, disabled]);
  ...
  return (
    <Wrapper styles={[fullWidth, height(24), padding(4), flex, jc("flex-start"), ai("center")]}>
      <Wrapper
        ...
        tabIndex={0}
      >
        {isCheckedProp && !isIndeterminate && <Icon width={sizeStyles[size].width} height={sizeStyles[size].height} icon="check" color="white" />}
        {isIndeterminate && <Icon width={sizeStyles[size].width} height={sizeStyles[size].height} icon="minus" color="white" />}
          <input
              type="checkbox"
      ...
      <Typography
          ...
          onClick={onChange}>
          {text}{isRequired && !!text?.length && <span style={{color: colors["red/05"]}}>*</span>}
      </Typography>
    </Wrapper>
  );
}
